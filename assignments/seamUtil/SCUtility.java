...
public class SCUtility {
  ...
  public static double[][] toEnergyMatrix(SeamCarver sc) {
    ...
    for (int col = 0; col < sc.width(); col++)
      for (int row = 0; row < sc.height(); row++) returnDouble[col][row] = sc.energy(col, row);
  ...
  public static Picture doubleToPicture(double[][] grayValues) {
    ...
    for (int col = 1; col < width - 1; col++) {
      for (int row = 1; row < height - 1; row++) {
        if (grayValues[col][row] > maxVal) maxVal = grayValues[col][row];
      }
    }

    if (maxVal == 0) return picture; // return black picture
    ...
    for (int col = 0; col < width; col++) {
      for (int row = 0; row < height; row++) {
        ...
        picture.set(
            col, row, new Color(normalizedGrayValue, normalizedGrayValue, normalizedGrayValue));
      }
    }
  ...
  public static Picture seamOverlay(Picture picture, boolean horizontal, int[] seamIndices) {
    ...
    for (int col = 0; col < width; col++)
      for (int row = 0; row < height; row++) overlaid.set(col, row, picture.get(col, row));

    // if horizontal seam, then set one pixel in every column
    if (horizontal) {
      for (int col = 0; col < width; col++) overlaid.set(col, seamIndices[col], Color.RED);
    }

    // if vertical, put one pixel in every row
    else {
      for (int row = 0; row < height; row++) overlaid.set(seamIndices[row], row, Color.RED);
    }
