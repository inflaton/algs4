...
public class BoggleGame extends JFrame {
  ...
  private static final int FOUND_WORDS_DISPLAY_COUNT =
      17; // how many rows to display for the two side columns
  private static final int ALL_WORDS_DISPLAY_COUNT =
      7; // how many rows to display for the middle all-words list
  ...
  private static final String[] LEVEL_DESCRIPTION = {
    "Nursery", "Shakespeare", "Algorithms 4/e", "Hard", "Impossible"
  };
  ...
  public BoggleGame(int rows, int cols) {
    ...
    layout.setHorizontalGroup(
        layout
            .createSequentialGroup()
            .addPreferredGap(
                LayoutStyle.ComponentPlacement.UNRELATED, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
            .addComponent(
                controlPanel,
                GroupLayout.PREFERRED_SIZE,
                GroupLayout.DEFAULT_SIZE,
  ...
  private void newGame() {
    ...
    if (gameDifficulty == NURSERY) {
      for (String word : validWords)
        if (nurseryDictionary.contains(word)) opponentFoundWords.add(word);
      ...
      for (String word : validWords)
        if (algs4Dictionary.contains(word)) opponentFoundWords.add(word);
    } else if (gameDifficulty == HARD) {
      for (String word : validWords)
        if (commonDictionary.contains(word) && StdRandom.bernoulli()) opponentFoundWords.add(word);
    } else if (gameDifficulty == IMPOSSIBLE) {
      for (String word : validWords) if (StdRandom.uniform(4) != 0) opponentFoundWords.add(word);
    }

    // opponent's score
    oppCurScore = 0;
    for (String word : opponentFoundWords) oppCurScore += scoreWord(word);
  ...
  private void endGame() {
    ...
    for (String s : validWords) {
      if (foundWords.contains(s)) indices[i++] = n;
      n++;
    }
    ...
    JOptionPane.showMessageDialog(
        this, winnerMessage + scoreMessage, "Game finished", JOptionPane.PLAIN_MESSAGE);
  }
  ...
  /**
   * Check the word entered in the text field or selected by clicks on the board Pressing ENTER or
   * clicking the Check Word button will call this.
   */
  private void checkWord() {
    ...
    else s = bp.getCurrentPath().toUpperCase();
    s = s.trim();
    if (s.equals("")) return;
  ...
  private class BoardPanel extends JPanel {
                  ...
                  for (int j = 0; j < path.length; j++) {
                    ...
                    // check for adjacency to the last cube in the path
                      ...
                      if (path[j - 1] >= cur - BOARD_COLS - 1
                          && path[j - 1] <= cur - BOARD_COLS + 1) path[j] = cur;
                      // next to (same row)
                      else if (path[j - 1] == cur - 1 || path[j - 1] == cur + 1) path[j] = cur;
                      // row below
    ...
    public void setBoard() {
      ...
      for (int i = 0; i < BOARD_ROWS; i++) {
        for (int j = 0; j < BOARD_COLS; j++) {
          ...
          if (letter == 'Q') cubes[i * BOARD_COLS + j].setText("Qu");
          else cubes[i * BOARD_COLS + j].setText("" + letter);
        }
      }
    }
     ...
     * @param pathIndex Current number of cubes searched (only differs from curChar if there is a q
     *     in string)
    ...
    private void dfs(String s, int curChar, int pathIndex, int i, int j) {
      ...
      if (curChar != 0 && s.charAt(curChar - 1) == 'Q' && s.charAt(curChar) != 'U') return;
      // increment character count if it is a 'U' after a 'Q' and keep searching
      if (curChar != 0 && s.charAt(curChar - 1) == 'Q' && s.charAt(curChar) == 'U') curChar += 1;
      if (curChar >= s.length()) {
        foundWord = true;
        return;
      }
    ...
    for (int i = 0; i < NUMBER_OF_LEVELS; i++) {
      difficultySelection[i] =
          new JRadioButtonMenuItem(
              LEVEL_DESCRIPTION[
                  i % LEVEL_DESCRIPTION.length]); // mod as a check against mismatched sizes
