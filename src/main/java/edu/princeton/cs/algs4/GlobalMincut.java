 ...
 * in a graph with non-negative edge weights. A <em>cut</em> is a partition of the vertices into two
 * nonempty subsets. An edge that has one endpoint in each subset of a cut is a <em>crossing
 * edge</em>. The weight of a cut is the sum of the weights of its crossing edges. A <em>global
 * minimum cut</em> whose weight is no larger than the weight of any other cut.
 *
 * <p>This is an implementation of <em>Stoer-Wagner's algorithm</em>. The constructor takes
 * <em>O</em>(<em>V</em> (<em>V</em> + <em>E</em>) log <em>V</em>) time, where <em>V</em> is the
 * number of vertices and <em>E</em> is the number of edges. The <em>weight</em> and <em>isCut</em>
 * methods take &Theta;(1) time. It uses &Theta;(<em>V</em>) extra space (not including the graph).
 *
...
public class GlobalMincut {
  ...
  /**
   * Returns {@code true} if the vertex {@code v} is one side of the mincut and {@code false}
   * otherwise. An edge <em>v</em>-<em>w</em> crosses the mincut if and only if <em>v</em> and
   * <em>w</em> have opposite parity.
   *
   ...
   * @throws IllegalArgumentException unless vertex {@code v} is between {@code 0 <= v < V}
   */
  ...
  /**
   * Makes a cut for the current edge-weighted graph by partitioning its vertices into two nonempty
   * subsets. The vertices connected to the vertex {@code t} belong to the first subset. Other
   * vertices not connected to {@code t} belong to the second subset.
   *
